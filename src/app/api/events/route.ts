...
2. **検索の最適化**: 複数の施設を調査する場合、個別に検索するのではなく、できるだけ1回のGoogle検索で済むようにクエリを工夫してください。例えば、「(施設名1 OR 施設名2) YYYY年MM月DD日 イベント」のように、OR演算子を使って検索をまとめてください。
3. **イベント特定**: 各施設のイベントを調査。複数ある場合は全て列挙。
4. **ピーク算出**: 開始1-2時間前を「開場前」、終了0-1時間後を「終演後」として24時間制数値で出力。
5. **混雑度評価(1-10)**: 「駅乗降客数に対する比率(相対)」と「来場者絶対数(絶対)」の最大値を基準に評価。

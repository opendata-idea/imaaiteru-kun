...
const Truck = ({ compact = false }: TruckProps) => {
  ...
  const TruckCar = ({ offsetX = 0 }: { offsetX?: number }) => (
    <g transform={`translate(${offsetX}, 0) scale(0.35)`}>
      {/* タイヤ（車体の後ろ） - 回転アニメーション */}
      <g className="animate-spin-wheel" style={{ transformOrigin: '130px 225px' }}>
        <circle cx="130" cy="225" r="30" fill={color} />
        <circle cx="130" cy="225" r="10" fill="white" />
      </g>
      <g className="animate-spin-wheel" style={{ transformOrigin: '200px 225px' }}>
        <circle cx="200" cy="225" r="30" fill={color} />
        <circle cx="200" cy="225" r="10" fill="white" />
      </g>
      <g className="animate-spin-wheel" style={{ transformOrigin: '440px 225px' }}>
        <circle cx="440" cy="225" r="30" fill={color} />
        <circle cx="440" cy="225" r="10" fill="white" />
      </g>
      <g className="animate-spin-wheel" style={{ transformOrigin: '510px 225px' }}>
        <circle cx="510" cy="225" r="30" fill={color} />
        <circle cx="510" cy="225" r="10" fill="white" />
      </g>
      
      {/* 車体の枠（1つの大きな長方形） */}
      <rect x="60" y="60" width="510" height="160" rx="25" fill="white" stroke={color} strokeWidth="6" />
      ...
      <line x1="385" y1="165" x2="530" y2="165" stroke={color} strokeWidth="5" />
  ...
  if (compact) {
    return (
      <div className="w-full overflow-hidden bg-pink-50 rounded-lg py-2">
        <svg width="100%" height="80" viewBox="-100 0 1500 200" preserveAspectRatio="xMidYMid meet">
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-gray-100 overflow-hidden">
      ...
      <div className="w-full max-w-full">
        <svg width="100%" height="200" viewBox="-100 0 1500 200" preserveAspectRatio="xMidYMid meet">
          {/* 6両のトラックを横並びに配置 */}
          <g className="animate-train">
            <TruckCar offsetX={0} />
            <TruckCar offsetX={190} />
